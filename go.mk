...
$(config_slaves) : configs/% : configs/0
	...
	$(ssh) $(user)@$(ip_addr) bash -c "if test -d /tmp/$(git_dir) ; then git -C /tmp/$(git_dir) pull ; else git -C /tmp clone $(this_repo_url) ; fi"
	$(scp) data/*.csv $(user)@[$(ip_addr)]:/tmp/$(git_dir)/data/
